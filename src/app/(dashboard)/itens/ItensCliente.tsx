...
function useCompendioLista<T extends { criado_por?: string | null }>(
  ...
  // Chave estável para a dependência do efeito — os call-sites passam um
  // array literal (ex: ['rarity', 'name_pt']), recriado a cada render.
  // Usar o array direto no array de dependências do useEffect reexecutava
  // o fetch a cada render (novas referências nunca são Object.is-iguais),
  // travando a tela num loop infinito de "carregando" — o bug real por
  // trás do "Itens não abre mais" (não era equipment_tools, que nenhuma
  // tela consulta).
  const ordenarPorChave = ordenarPor.join(',')
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      for (const campo of ordenarPorChave.split(',')) query = query.order(campo)
      ...
      if (cancelado) return
      if (error) {
        toast.error(`Erro ao carregar ${tabela}: ${error.message}`)
        setLista([])
      } else {
  ...
  }, [tabela, campos, ordenarPorChave, campanhaId, ehDM])
